...
class swiftBaseClass:
    ...
    def make_cbs_lim(self, uumid, amt, ccy, vdate, scenario="", creation_time=None):
        return {
            "SCENARIO": scenario,
            "ACCT": random.randint(10**11, 999999999999),
            "CURRENCY": ccy,
            "AC_HOLDER_NAME": self.random_string("MR."),
            "BANK_NAME": random.choice(['IDFC FIRST BANK']),
            "BRANCH_NAME": random.choice(['Trade Finance']),
            "VALUE_DATE": vdate,
            "TRANDATE": datetime.today().strftime("%d-%m-%Y"),
            "NARR": uumid[15:35],
            "TRACE_ID": random.randint (10000000,99999999),
            "TXN_TYPE" : random.choice(['1']),
            "TRANCODE" : str(uuid.uuid4()),
            "TRAN_CODE_DESC" : self.random_string("CTA."),
            "DR_CR_FLAG" : random.choice(['DR']),
            "AMOUNT": amt,
            "POSTTIME": creation_time if creation_time else datetime.today().strftime("%H:%M:%S")
        }
